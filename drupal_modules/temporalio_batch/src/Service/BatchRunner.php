...
class BatchRunner {
  ...
  protected function baseUrl(): string {
    return $this->common->sidecarBaseUrl();
  }

  protected function secret(): string {
    return $this->common->hmacSecret();
  }
  ...
  public function start(array $batch): string {
    ...
    $payload = [
      'batchId' => $id, 'title' => $batch['title'] ?? 'Batch',
      'finishedCallback' => isset($batch['finished']) ? $this->spec($batch['finished']) : NULL,
      'operations' => $ops,
    ];
  ...
  protected function spec($callable): array {
    if (is_string($callable)) {
      return ['type' => 'function', 'value' => $callable];
    }
