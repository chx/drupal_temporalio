...
class BatchCallbackController extends ControllerBase {
  ...
  public function execute(Request $request): JsonResponse {
    ...
    $op = $data['op'] ?? NULL;
    if (!$op) {
      return new JsonResponse(['error' => 'missing op'], 400);
    }
    try {
      $res = $this->invoke($op['callable'] ?? [], $op['args'] ?? []);
  ...
  protected function invoke(array $spec, array $args) {
    switch ($spec['type'] ?? '') {
      case 'function':
        return call_user_func_array($spec['value'], $args);

      case 'static_method':
        return call_user_func_array([$spec['class'], $spec['method']], $args);
